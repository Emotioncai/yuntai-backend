...
public class StatisticService {
    ...
    public static BigDecimal getGMV(String date) {
        try {
            ...
            Connection connection = DriverManager.getConnection(DATABASE.CLICKHOUSE_URL, DATABASE.CLICKHOUSE_USERNAME, DATABASE.CLICKHOUSE_PASSWORD);
    ...
    public static List<GeneralStatistic> getProvinceStatistics(String date) {
        try {
            ...
            selectStatement.setString(1, date);
            ResultSet resultSet = selectStatement.executeQuery();
            ArrayList<GeneralStatistic> result = new ArrayList<>();
            while (resultSet.next()) {
                GeneralStatistic provinceStatistics = new GeneralStatistic();
    ...
    public static List<GeneralStatistic> getKeywordStatistics(String date) {
        ...
        try {
            Class.forName(DATABASE.CLICKHOUSE_DRIVER);
            Connection connection = DriverManager.getConnection(DATABASE.CLICKHOUSE_URL);
            PreparedStatement selectStatement = connection.prepareStatement(sql);
            selectStatement.setString(1, date);
            ResultSet resultSet = selectStatement.executeQuery();
            ArrayList<GeneralStatistic> result = new ArrayList<>();
            while (resultSet.next()) {
                GeneralStatistic keywordStatistics = new GeneralStatistic();
    ...
    public static List<GeneralStatistic> getProductStatisticsGroupByCategory3(String date) {
        ...
        try {
            ...
            while (resultSet.next()) {
                GeneralStatistic categoryStatistics = new GeneralStatistic();
    ...
    public static List<GeneralStatistic> getProductStatsByTrademark(String date) {
        ...
        try {
            ...
            List<GeneralStatistic> result = new ArrayList<>();
            while (resultSet.next()) {
                GeneralStatistic trademarkStatistics = new GeneralStatistic();
    ...
    public static Page<TrafficStatistic> getTrafficStatistics(Integer page, Integer limit, String dt, Integer recentDays) {
        ...
        try {
            ...
            ResultSet resultSet = selectStatement.executeQuery();
            List<TrafficStatistic> trafficStatistics = new ArrayList<>();
            int total = 0;
            while (resultSet.next()) {
    ...
    public static Map<String, Object> getPagePathCount(TrafficStatistic trafficStatistic) {
        ...
        try {
            ...
            List<PagePath> pagePaths = new ArrayList<>();
            ResultSet resultSet = selectStatement.executeQuery();
            while (resultSet.next()) {
